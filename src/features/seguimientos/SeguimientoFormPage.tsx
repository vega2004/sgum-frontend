...
export function SeguimientoFormPage() {
  const { id = '' } = useParams(); const navigate = useNavigate(); const { showSuccess, showError } = useNotification();
  ...
  async function submit(values: SeguimientoInput) { try { await createSeguimiento(values); showSuccess({ title: 'Seguimiento registrado correctamente.', description: 'La acción quedó registrada en el sistema.' }); navigate(`/expedientes/${id}`); } catch (error) { const friendly = normalizeApiError(error); showError({ title: 'No fue posible guardar el seguimiento.', description: friendly.description }); } }
